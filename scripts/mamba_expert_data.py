...
from rl.algorithms.mamba_expert_data import Mamba
...
def create_learner(envid, seed, policy0, vfn0, 
                    ...
                    policy_name=None, 
                    vfn_name=None):
    # Try to load the initial policy and value functioni
...
def main(c):
    ...
    policy, vfn = create_learner(c['mdp']['envid'], c['seed'], policy, vfn, 
                                path='expert_diversity_test/cp-0.9-8-max-F/0/saved_policies/',
                                policy_name='learner_policy_best')
