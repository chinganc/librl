        ...
        with timed('Pretraining'):
            ...
            for _ in range(self._n_pretrain_itrs):
                for k, expert in enumerate(self.experts):
                    ...
                    ro = self.merge(ros)
                    # ro_experts.append({'obs':ro['obs_short'],'acs':ro['acs']})
                    _, ev0, ev1 = self.aes[k].update(ro, n_steps=2000) 
                    print('pretrain explained variances', ev0, ev1)
                    #self.policy.update(ro['obs_short'])

                # import pickle
                # pickle.dump(ro_experts, open('experts_ro.p', 'wb'))
    ...
    def update(self, ros, agents):  # agents are behavior policies
        ...
        with timed('Update oracle'):
            ...
            for k, ro_exp in enumerate(ro_exps):
                if len(ro_exp)>0:
                    _, ev0, ev1 = self.aes[k].update(ro_exp, n_steps=2000)
                    EV0.append(ev0)
                    EV1.append(ev1)
            ## Update oracle
        ...
        for ro in ro_pol:
            ...
            rws.append(ro.rws)
            vk = []
            for i in range(len(self.vfn.vfns)):
                vk.append(self.vfn.vfns[i](ro.obs))    
            vks.append(vk)
        #import pdb; pdb.set_trace()
        import pickle
        data = {'obs':obs, 'acs':acs, 'rws':rws, 'vks':vks}
        pickle.dump(data, open('value_data.p', 'wb'))
