...
def load_policy(filename):
    ...
    #obs_bo = tf.compat.v1.placeholder(tf.float32, [None, None])
    #a_ba = build_policy(obs_bo)
    #policy_fn = tf_util.function([obs_bo], a_ba)
    from functools import partial
    return  partial(policy_fn, data=data)



def policy_fn(obs_bo, data):
    output_bo = build_policy(obs_bo, data)
    return output_bo.numpy()

def build_policy(obs_bo, data):
   
    obs_bo = tf.cast(obs_bo, tf.float32)
